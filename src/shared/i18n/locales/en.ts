/**
 * The default locale and the **source of truth for the key type**: every other
 * dictionary is typed against this one, and `t()` only accepts paths that exist
 * here. Add a key here first.
 *
 * Structure: group by feature (`auth`, `lessons`, `settings`), keep truly
 * generic strings in `common`.
 */
export const en = {
  common: {
    appName: 'Fox Academy',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    done: 'Done',
    close: 'Close',
    retry: 'Retry',
    search: 'Search',
    loading: 'Loading…',
    empty: 'Nothing here yet',
    error: 'Something went wrong',
    back: 'Back',
    continue: 'Continue',
  },
  addons: {
    title: 'Capabilities',
    hint: 'Extra things this person can do, beyond teaching.',
    none: 'No extra capabilities',
    inviteTutors: {
      title: 'Invite tutors',
      description: 'Can invite new tutors to the school by email.',
    },
    manageStudents: {
      title: 'Manage students',
      description: 'Can add, edit and remove students. A tutor only their own.',
    },
    broadcastAnnouncements: {
      title: 'Announcements',
      description: 'Can send a message to everyone in the school.',
    },
  },
  announcement: {
    compose: 'Write an announcement',
    message: 'Message',
    placeholder: 'What should everyone know?',
    send: 'Send to everyone',
    tooShort: 'Write at least a few words.',
    failed: 'Could not send the announcement. Please try again.',
    sent: 'Announcement sent to {{count}} people.',
    explanation: 'This appears in the News tab for every member of the school.',
  },
  notificationSettings: {
    announcementsTitle: 'School announcements',
    announcementsHint: 'Sent by your school to everyone. Delivered as a notification when the app is closed.',
    pushDenied: 'Notifications are switched off for Fox Academy in your device settings.',
    pushNotConfigured: 'Announcements appear in News. Push delivery is not set up in this build.',
    pushUnsupported: 'Announcements appear in News. Push delivery needs the mobile app.',
    pushReady: 'This device will be notified.',
    title: 'Notifications',
    lessonReminders: 'Remind me before a lesson',
    lessonRemindersHint: 'An automatic notification before each scheduled lesson.',
    leadTime: 'How long before',
    minutesShort: {
      one: '{{count}} min',
      other: '{{count}} min',
    },
    saveFailed: 'Could not save. Your change was reverted.',
    unsupported: 'Lesson reminders are available in the mobile app.',
  },
  support: {
    title: 'Support',
    compose: 'Contact support',
    message: 'Message',
    placeholder: 'What went wrong, or what do you need?',
    explanation: 'Your message is sent to us along with your account details.',
    send: 'Send',
    tooShort: 'Please describe the problem in a little more detail.',
    failed: 'Could not send your message. Please try again.',
    sent: 'Thanks — we have your message and will get back to you.',
  },
  reminders: {
    title: 'Lesson with {{name}}',
    body: 'Starts at {{time}} · {{subject}}',
    permissionDenied: 'Notifications are turned off for this app. Enable them in system settings to get reminders.',
  },
  studentsAdmin: {
    ownHint: 'The students you teach.',
    allHint: 'Everybody the school teaches.',
    title: 'Students',
    empty: 'No students yet',
    emptyHint: 'Add the people you teach; lessons are booked against them.',
    add: 'Add a student',
    edit: 'Edit student',
    name: 'Name',
    namePlaceholder: 'First and last name',
    subject: 'Usual subject',
    paidLessons: 'Paid lessons left',
    nameRequired: 'Enter a name.',
    save: 'Save',
    remove: 'Remove student',
    removeConfirm: 'Remove {{name}}? Their lessons are deleted too.',
    removeCancel: 'Keep',
    failed: 'Could not save. Please try again.',
    ownOnly: 'You can edit the students you teach.',
  },
  school: {
    title: 'School management',
    tutors: 'Tutors',
    tutorsEmpty: 'Nobody else yet. Invite a colleague and they appear here.',
    you: '{{name}} (you)',
    invitations: 'Invitations',
    invitationsHint: 'People who have been invited but have not joined yet.',
    noInvitations: 'No invitations sent',
    inviteTutor: 'Invite a tutor',
    sendInvite: 'Send invitation',
    inviteExplanation: 'We will email a link. Opening it on their phone starts registration into this school.',
    invalidEmail: 'Enter a valid email address.',
    inviteFailed: 'Could not send the invitation. Please try again.',
    status: {
      pending: 'Pending',
      accepted: 'Joined',
      expired: 'Expired',
    },
  },
  invite: {
    title: 'Join {{school}}',
    subtitle: '{{name}} invited you to {{school}} on Fox Academy.',
    emailFixed: 'Taken from your invitation and cannot be changed.',
    yourName: 'Your name',
    namePlaceholder: 'How students will see you',
    nameRequired: 'Enter your name.',
    passwordHint: 'At least 8 characters.',
    passwordTooShort: 'Use at least 8 characters.',
    join: 'Join the school',
    failed: 'Could not complete registration. Please try again.',
    invalidTitle: 'This invitation is no longer valid',
    invalidBody: 'The link may have expired or already been used. Ask the school for a new one.',
  },
  settings: {
    title: 'Settings',
    appearance: {
      title: 'Appearance',
      accent: 'Accent color',
      system: 'System',
      light: 'Light',
      dark: 'Dark',
    },
    tour: {
      title: 'Interface tour',
      hint: 'A short walk through the calendar, news and settings.',
      action: 'Show me around',
    },
    navigation: {
      title: 'Bottom navigation',
      hint: 'Choose which tabs appear and in what order.',
      moveUp: 'Move up',
      moveDown: 'Move down',
      toggle: 'Show or hide',
      reset: 'Reset to default',
    },
    language: {
      title: 'Language',
      /** `null` override — follow the device language. */
      system: 'Device language',
      systemShort: 'Auto',
    },
  },
  auth: {
    signIn: 'Sign in',
    signOut: 'Sign out',
    email: 'Email',
    password: 'Password',
    forgotPassword: 'Forgot your password?',
    signInTitle: 'Welcome back',
    signInSubtitle: 'Sign in to manage your lessons and students.',
    emailPlaceholder: 'you@school.com',
    signingIn: 'Signing in…',
    signInFailed: 'Could not sign in. Please try again.',
    sessionExpired: 'Your session ended. Please sign in again.',
    createAccount: 'Create an account',
  },
  join: {
    title: 'Get started',
    subtitle: 'Two ways in, depending on whether the school already exists.',
    openTitle: 'Open your own school',
    openBody:
      'For an independent tutor or a school starting out. You become its admin and can invite colleagues later.',
    openAction: 'Open a school',
    existingTitle: 'Join an existing school',
    existingBody: 'For a tutor whose school is already using Fox Academy.',
    existingAction: 'Join a school',
    haveAccount: 'Already have an account?',
    existing: {
      title: 'Your school invites you',
      body: 'Schools choose who teaches under their name, so joining one happens by invitation rather than by filling in a form.',
      stepsTitle: 'How it works',
      step1: 'Ask an admin at your school to invite you, and give them this email address.',
      step2: 'They send the invitation from Fox Academy, and it arrives in your inbox.',
      step3: 'Open the link in that email — it brings you straight here with your school already filled in.',
      note: 'The link works once and expires after a few days. If it has expired, just ask for another.',
      haveLink: 'I already have an account',
    },
    school: {
      title: 'Open your school',
      aboutSchool: 'First, the school itself.',
      aboutYou: 'Now your own account — you will be its admin.',
      nameLabel: 'School name',
      namePlaceholder: 'Fox Academy',
      nameHint: 'What your students already call you. Teaching alone? Your own name is fine.',
      nameRequired: 'Please give the school a name.',
      timezoneLabel: 'Time zone',
      timezoneHint: 'Taken from this device. Lesson times follow it; you can change it in school settings.',
      yourName: 'Your name',
      yourNamePlaceholder: 'Olha Kovalenko',
      yourNameRequired: 'Please enter your name.',
      emailInvalid: 'Please enter a valid email address.',
      passwordTooShort: 'Use at least 8 characters.',
      passwordHint: 'At least 8 characters.',
      failed: 'Could not create the school. Please try again.',
      create: 'Create school',
    },
  },
  tutorial: {
    back: 'Back',
    next: 'Next',
    skip: 'Skip',
    done: 'Got it',
    steps: {
      calendar: {
        title: 'Your week, by the hour',
        body: 'The calendar opens on today. Every lesson shows who it is with, so a glance is enough.',
      },
      views: {
        title: 'Move around',
        body: 'Step back and forward, or jump to today. One day, three days or a whole month — whichever suits, in settings.',
      },
      booking: {
        title: 'Book a lesson',
        body: 'This adds a lesson to your own calendar. Pick the student, the time and how long it runs.',
      },
      news: {
        title: 'What needs you',
        body: 'Lessons to confirm, students running out of paid lessons, announcements from the school — all in one place.',
      },
      more: {
        title: 'Everything else',
        body: 'Language, appearance, reminders and which tabs you see all live in settings.',
      },
    },
  },
  notes: {
    add: 'Add a note',
    placeholder: 'What is worth remembering?',
    save: 'Save note',
    remove: 'Remove note',
    failed: 'Could not save that. Please try again.',
    byline: '{{author}} · {{when}}',
    studentTitle: 'Notes',
    studentEmpty: 'Nothing noted yet. Preferences, contacts, anything worth not re-asking.',
    lessonTitle: 'Lesson notes',
    lessonEmpty: 'Nothing noted for this lesson yet.',
  },
  files: {
    lessonTitle: 'Material',
    lessonEmpty:
      'Nothing attached yet. A worksheet, the slides, a photo of the board — whatever is worth keeping with the lesson.',
    open: 'Open {{name}}',
    openFailed: 'Could not open that file.',
    openUnavailable: 'Files are not stored in a test build — only their names are.',
    share: 'Send {{name}}',
    title: 'Files',
    empty: 'No files yet. Agreements, reports, worksheets — anything worth keeping.',
    add: 'Add a file',
    remove: 'Remove file',
    meta: '{{size}} · {{when}}',
    failed: 'Could not add that file. Please try again.',
    failedType: 'That kind of file is not accepted. Images, PDFs, Word and Excel documents are.',
    failedTooLarge: 'That file is too large. The limit is 10 MB.',
    failedOffline: 'Could not reach the server. Check your connection and try again.',
  },
  meetings: {
    title: 'Online lessons',
    provider: {
      label: 'Where you teach',
      NONE: 'In person',
      ZOOM: 'Zoom',
      GOOGLE_MEET: 'Google Meet',
      JITSI: 'Jitsi',
    },
    hint: {
      NONE: 'Lessons are booked without a link. Choose a service to add one automatically.',
      ZOOM: 'Every lesson gets a link to your own Zoom room. Zoom will not create a room per lesson without connecting your account, which the app cannot do yet.',
      GOOGLE_MEET: 'Every lesson gets a link to your own Meet room. Google will not create a room per lesson without connecting your account, which the app cannot do yet.',
      JITSI: 'Every lesson gets its own room, created automatically. Nothing to set up and no account needed — anyone with the link can join, so send it only to your students.',
    },
    example: {
      ZOOM: 'https://us05web.zoom.us/j/1234567890',
      GOOGLE_MEET: 'https://meet.google.com/abc-defg-hij',
      JITSI: '',
    },
    roomLabel: 'Your meeting room',
    roomHint: 'Open your meeting room and copy the address from the browser.',
    problem: {
      missing: 'Add the address of your meeting room.',
      notAUrl: 'That is not a web address.',
      notHttps: 'The address has to start with https://',
      hasCredentials: 'The address cannot carry a username or password.',
      wrongHost: 'That address is not on the service you chose.',
    },
    saveFailed: 'Could not save that. Please try again.',
    onlineLesson: 'Online lesson',
    join: 'Join',
    share: 'Share link',
    cannotOpen: 'Nothing on this device can open that link.',
    cannotShare: 'Could not share that link.',
    connect: 'Connect',
    connected: 'Connected',
    notConnected: 'Not connected — lessons will use the address below, if you give one.',
    disconnect: 'Disconnect',
    connectedToast: 'Connected. New lessons will each get their own room.',
    connectFailed: 'Could not connect. Please try again.',
    notConfigured: 'This server cannot create rooms for you yet. Paste your own meeting room below and every lesson will use it.',
    roomHintFallback: 'Optional now that you are connected — used only if the service cannot be reached when a lesson is booked.',
    changeNote: 'Applies to lessons booked from now on. Links already sent stay as they are.',
  },
  reports: {
    title: 'Reports',
    subtitle: 'What was taught, and how it went.',
    period: {
      label: 'Period',
      week: '7 days',
      month: '30 days',
      quarter: '90 days',
      year: '365 days',
    },
    scope: {
      mine: 'Your lessons',
      school: 'The whole school',
      tutor: '{{name}}’s lessons',
    },
    lessons: {
      title: 'Lessons',
      completed: 'Taught',
      cancelled: 'Cancelled',
      scheduled: 'Still to come',
      hours: 'Hours taught',
      students: 'Students taught',
    },
    attendance: {
      title: 'Attendance',
      rate: 'Turned up',
      present: 'Present',
      late: 'Late',
      absentExcused: 'Excused',
      absentUnexcused: 'No-show',
      none: 'Nothing has been marked in this period.',
      hint: 'Late counts as attended — they were taught.',
      unwritten: {
        one: '{{count}} lesson has no register, so it is not in the rate.',
        other: '{{count}} lessons have no register, so they are not in the rate.',
      },
      noneButTaught: {
        one: '{{count}} lesson was taught but nobody was marked, so there is no rate yet.',
        other: '{{count}} lessons were taught but nobody was marked, so there is no rate yet.',
      },
    },
    grades: {
      title: 'Marks',
      classic: 'Average mark',
      percentage: 'Average percentage',
      descriptive: 'Written marks',
      count: 'Marks given',
      none: 'No marks in this period.',
    },
    bySubject: 'By subject',
    byTutor: 'By tutor',
    noSubject: 'No subject',
    unnamedTutor: 'Former member',
    breakdownRow: '{{lessons}} · {{hours}}',
    empty: 'Nothing was taught in this period.',
    failed: 'Could not load the report.',
    retry: 'Try again',
    hoursValue: '{{hours}} h',
  },
  debtors: {
    title: 'Who owes lessons',
    subtitle: 'Students taught beyond what they have paid for.',
    warnEarly: 'Warn me a lesson early',
    warnEarlyHint: 'Also list anyone with a single lesson left.',
    owing: {
      one: '{{count}} lesson to pay for',
      other: '{{count}} lessons to pay for',
    },
    runningOut: 'Nothing left on their package',
    booked: {
      one: '{{count}} lesson booked',
      other: '{{count}} lessons booked',
    },
    nothingBooked: 'Nothing booked',
    lastTaught: 'Last taught {{when}}',
    neverTaught: 'No lesson written up yet',
    formerTutor: 'Former member',
    remind: 'Remind',
    messageOwing: 'Hello! {{name}} has had {{count}} lesson(s) beyond the last package. Shall we top it up before the next one?',
    messageRunningOut: 'Hello! {{name}} has no lessons left on the current package. Shall we top it up before the next one?',
    cannotShare: 'Could not open the share sheet.',
    empty: 'Nobody owes anything. Every student has lessons left.',
    failed: 'Could not load the list.',
  },
  gradebookSettings: {
    title: 'Marking',
    enabled: 'I mark my students’ work',
    enabledHint: 'Shows marks, averages and the gradebook on each student.',
    offHint:
      'Marks are hidden, not deleted. Attendance, lesson topics and homework are unaffected, and anything already marked comes back if you switch this on.',
  },
  groups: {
    title: 'Groups',
    tab: 'Groups',
    students: 'Students',
    create: 'New group',
    createAction: 'Create group',
    edit: 'Edit group',
    name: 'Name',
    nameHint: 'B1 Tuesdays',
    subject: 'Subject',
    level: 'Level',
    levelHint: 'B1, Beginners, Year 9…',
    members: 'Students in this group',
    membersEmpty: 'Nobody yet. Add students and they join every lesson this group has.',
    addMember: 'Add a student',
    removeMember: 'Take out of group',
    nobodyLeft: 'Everyone you teach is already in this group.',
    remove: 'Dissolve group',
    removeHint:
      'Removes the group and its lessons. The students, their marks and their attendance are kept.',
    failed: 'Could not save that. Please try again.',
    empty: 'No groups yet',
    emptyHint:
      'A group is a set of students taught together. Book one lesson and everybody in it is marked from a single register.',
    count: {
      one: '{{count}} student',
      other: '{{count}} students',
    },
  },
  subjects: {
    title: 'Subjects',
    hint: 'What your school teaches. Everything a student, a group or a lesson studies is chosen from this list.',
    empty: 'Nothing here yet. Add the subjects your school teaches.',
    emptyForTutor: 'Your school has not set up its subjects yet. An admin adds them in school management.',
    add: 'Add a subject',
    name: 'Name',
    namePlaceholder: 'Mathematics',
    none: 'Not set',
    retired: 'No longer taught',
    retiredCount: {
      one: '{{count}} no longer taught',
      other: '{{count}} no longer taught',
    },
    showRetired: 'Show what you no longer teach',
    hideRetired: 'Hide those',
    rename: 'Rename',
    retire: 'Stop teaching this',
    retireHint:
      'It stops being offered anywhere. Lessons already taught keep it and read exactly as they do now.',
    restore: 'Teach it again',
    exists: 'Your school already teaches that.',
    existsRetired:
      'You used to teach that. Bring it back rather than adding a second one with the same name — the old lessons are attached to it.',
    bringBack: 'Bring it back',
    inUseTitle: 'Move these first',
    inUseHint:
      'A subject can only be retired once nothing is studying it. Reassign what is listed here, then try again.',
    inUseStudents: 'Students',
    inUseGroups: 'Groups',
    inUseLessons: {
      one: '{{count}} lesson still booked',
      other: '{{count}} lessons still booked',
    },
    keepsLessons: {
      one: 'Keeps {{count}} lesson already taught',
      other: 'Keeps {{count}} lessons already taught',
    },
  },
  errors: {
    loadSchedule: 'Could not load your schedule. Check your connection and pull to refresh.',
    bookLesson: 'That lesson was not saved. Please try booking it again.',
    saveLesson: 'Could not save that change to the server.',
    loadStudents: 'Could not load your students.',
    loadSubjects: 'Could not load your school’s subjects.',
    saveSubject: 'Could not save that subject.',
    generic: 'Something went wrong. Please try again.',
  },
  gradebook: {
    attendance: {
      present: 'Was here',
      late: 'Came late',
      absentExcused: 'Excused absence',
      absentUnexcused: 'Did not come',
      shortPresent: 'Here',
      shortLate: 'Late',
      shortExcused: 'Excused',
      shortUnexcused: 'No-show',
    },
    kind: {
      classic: 'Mark',
      percentage: 'Percent',
      descriptive: 'In words',
    },
    journal: {
      taughtBy: 'Taught by {{name}}',
      open: 'Write up this lesson',
      register: 'Register',
      registerEmpty: 'Nobody to mark — this group has no students yet.',
      groupOf: {
        one: '{{count}} student',
        other: '{{count}} students',
      },
      attendance: 'Did they turn up?',
      attendanceHint:
        'An excused absence gives the slot back. A no-show is still charged — the hour was held.',
      topic: 'What you covered',
      topicHint: 'Past simple, questions',
      homework: 'Homework',
      homeworkHint: 'Workbook p. 34',
      homeworkDone: 'Was it done?',
      homeworkUnchecked: 'Not checked',
      homeworkYes: 'Done',
      homeworkNo: 'Not done',
      homeworkDoneHint:
        'Usually answered at the start of the next lesson. Leave it unchecked until then.',
      save: 'Save write-up',
      failed: 'Could not save that. Please try again.',
      marks: 'Marks from this lesson',
      marksEmpty: 'No marks given in this lesson.',
      needsWriteUp: 'Not written up',
      writtenUp: 'Written up',
    },
    grade: {
      add: 'Add a mark',
      correct: 'Correct this mark',
      remove: 'Remove mark',
      kind: 'How you are marking',
      value: 'Mark',
      valuePercent: 'Percent',
      words: 'The mark, in words',
      remark: 'Remark',
      commentHint: 'What went well, what to work on',
      category: 'What it was for',
      categoryHint: 'Speaking, homework, unit test…',
      weight: 'Counts as',
      weightHint: 'A weight of 2 counts this mark twice in the average.',
      weightBadge: {
        one: 'counts once',
        other: 'counts {{count}}×',
      },
      descriptiveHint: 'A mark in words is kept in the record and left out of the average.',
      byline: '{{author}} · {{when}}',
      uncategorised: 'Mark',
      negative: 'A mark cannot be negative.',
      overHundred: 'A percentage cannot be over 100.',
      rejected: "Could not save that mark. Check the value against your school's scale.",
      failed: 'Could not save that. Please try again.',
      studentTitle: 'Marks',
      studentEmpty: 'No marks yet. Tests, homework, or just a note on how it is going.',
    },
    progress: {
      title: 'How it is going',
      empty: 'Nothing recorded yet. Write up a lesson and this fills in.',
      average: 'Average mark',
      averagePercent: 'Average',
      attendance: 'Attendance',
      taught: 'Lessons taught',
      fromMarks: {
        one: 'from {{count}} mark',
        other: 'from {{count}} marks',
      },
      ofMarked: {
        one: 'of {{count}} marked',
        other: 'of {{count}} marked',
      },
      ofTotal: {
        one: 'of {{count}} booked',
        other: 'of {{count}} booked',
      },
      missed: {
        one: '{{count}} no-show, charged',
        other: '{{count}} no-shows, charged',
      },
    },
  },
  library: {
    title: 'My files',
    hint: 'Your own teaching material — worksheets, scans, plans. Kept apart from what you hold about a student.',
    files: 'Files',
    empty: 'Nothing here yet. Add a worksheet and you can send it to anyone in two taps.',
  },
  studentDetail: {
    lessons: 'Lessons',
    lessonsEmpty: 'No lessons yet.',
    noteCount: {
      one: '{{count}} note',
      other: '{{count}} notes',
    },
    status: {
      scheduled: 'Scheduled',
      completed: 'Took place',
      cancelled: 'Cancelled',
    },
    notFound: 'That student is no longer on your list.',
    openLesson: 'Notes for this lesson',
  },
  time: {
    justNow: 'Just now',
    minutesAgo: {
      one: '{{count}} minute ago',
      other: '{{count}} minutes ago',
    },
    hoursAgo: {
      one: '{{count}} hour ago',
      other: '{{count}} hours ago',
    },
    daysAgo: {
      one: 'Yesterday',
      other: '{{count}} days ago',
    },
  },
  tabs: {
    calendar: 'Calendar',
    students: 'Students',
    news: 'News',
    more: 'More',
  },
  news: {
    title: 'News',
    empty: 'No news yet',
    emptyHint: 'Announcements, payment reminders and lessons to confirm show up here.',
    markAllRead: 'Mark all as read',
    unread: {
      zero: 'All caught up',
      one: '{{count}} unread',
      other: '{{count}} unread',
    },
    actions: {
      markHeld: 'Took place',
      markMissed: 'Did not happen',
      writeUp: 'Write up',
    },
    kinds: {
      adminAnnouncement: {
        title: 'Announcement',
        body: '{{text}}',
      },
      tutorJoined: {
        title: 'New tutor joined',
        body: '{{personName}} joined the school as a {{text}} tutor.',
      },
      paymentRunningOut: {
        title: 'Paid lessons running out',
        body: {
          one: '{{studentName}} has {{count}} paid lesson left.',
          other: '{{studentName}} has {{count}} paid lessons left.',
        },
      },
      lessonStartingSoon: {
        title: 'Lesson starting soon',
        body: 'Lesson with {{studentName}} at {{time}}.',
      },
      lessonNeedsConfirmation: {
        title: 'Did this lesson take place?',
        body: 'Lesson with {{studentName}} at {{time}} is still unconfirmed.',
      },
    },
  },
  calendarSettings: {
    title: 'Calendar settings',
    view: 'View',
    day: 'Day',
    threeDays: '3 days',
    month: 'Month',
  },
  filters: {
    title: 'Filters',
    calendars: 'Calendars',
    calendarsHint: 'Choose whose schedule is shown on your calendar.',
    visibleCount: {
      zero: 'No calendars shown',
      one: '{{count}} calendar shown',
      other: '{{count}} calendars shown',
    },
  },
  event: {
    add: 'New lesson',
    student: 'Student',
    newStudent: 'Add a new student',
    pickExisting: 'Pick from my students',
    newStudentName: 'Student name',
    lessonsLeft: {
      zero: 'no lessons left',
      one: '{{count}} lesson left',
      other: '{{count}} lessons left',
    },
    ownCalendarNote: 'Lessons are added to your own calendar.',
    studentPlaceholder: 'Who is the lesson with?',
    subject: 'Subject',
    date: 'Date',
    startTime: 'Starts at',
    duration: 'Duration',
    minutes: '{{count}} min',
    create: 'Add lesson',
    missingStudent: 'Enter a student name.',
    taught: 'Who is taught',
    forStudent: 'One student',
    forGroup: 'A group',
    group: 'Group',
    missingGroup: 'Pick a group first.',
  },
  calendar: {
    title: 'Calendar',
    today: 'Today',
    previous: 'Previous',
    next: 'Next',
    empty: 'No lessons on this day',
  },
  more: {
    title: 'More',
    school: 'School management',
    students: 'Students',
    support: 'Support',
    settings: 'Settings',
    files: 'My files',
    preferences: 'Preferences',
    account: 'Account',
  },
  lessons: {
    title: 'Lessons',
    count: {
      zero: 'No lessons scheduled',
      one: '{{count}} lesson',
      other: '{{count}} lessons',
    },
    nextWith: 'Next lesson with {{name}}',
  },
  students: {
    title: 'Students',
    count: {
      zero: 'No students yet',
      one: '{{count}} student',
      other: '{{count}} students',
    },
  },
} as const;

/** The canonical dictionary shape. Other locales are partials of this. */
export type AppDictionary = typeof en;
